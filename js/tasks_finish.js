...
function createButtons(task){
    ...
    // set btn-edit
    btn_edit.setAttribute('class', 'icon-finishTask mr-2');
    btn_edit.setAttribute('onclick', `finishTask("${task.id}")`)
...
function renderTasks(){
    let taskList = document.getElementById("taskListFinish")
    taskList.innerHTML = '' 
    for(let task of tasks){
        ...
        newTask.appendChild(createButtons(task));
...
async function deleteTask(id) {
    await db.collection("tasks_finish").doc(id).delete()
    readTasks(filter_id, id_user)
}

async function editTask(id) {
    let edit_task = document.getElementById('edit_task')
    let db_editTask = await db.collection("tasks_finish").doc(id).get()
...
async function saveTask() {
    ...
    if(title_update == '' || title_update == ' '){
        await db.collection("tasks_finish").doc(save_id).delete()
    }else{
        await db.collection("tasks_finish").doc(save_id).update({
            title: title_update,
            data: new Date(),
        })
    }
...
async function finishTask(id){
    ...
    li.classList.remove('icon-finishTask')
    li.classList.add('icon-editTask')
    const title = (await db.collection("tasks_finish").doc(id).get()).data().title
